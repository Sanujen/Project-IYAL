...
2. Make sure you have internet connection before using the Inference
   class
3. The model can be created with a custom path specified for storing the
   model. If a custom path is not provided, the model will be saved in
   the default directory, which is the absolute path of the directory
   where the ``Inference`` class is located, appended with
...
5. The ``Inference`` class initializes by checking the cache directory
...
3. Translation: https://github.com/ssut/py-googletrans
   ...
   - This is an unofficial library using the web API of
